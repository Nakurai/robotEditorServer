...
/**
* [Asynchronous] Executes all defined researches on the web for each sentences in the user input
...
WebSearcher.prototype.go = function(userInputSentences){
	...
	var nbSentences = userInputSentences.length;
	var deferred = this.q.defer();
	var sentences = userInputSentences;
	var nbAnalyzed = 0;

	for(var cpt=0; cpt<nbSentences; cpt++){
		var index = cpt;
		this.analyzeOneNeedle(sentences[cpt].needle).then(

			function(result){
				
				sentences[nbAnalyzed].img = result.img;
				sentences[nbAnalyzed].web = result.web;

				// When the last sentence has been analyzed, the deferred is resolved
				nbAnalyzed++;
				if(nbAnalyzed === nbSentences){
					console.log("webseaches completed for "+nbAnalyzed+" sentences");
...
* @param {needle:String} some words to search
...
WebSearcher.prototype.analyzeOneNeedle = function(needle){
	...
	// If another search engine is used, just add the function in the q.all parameter and concatenate. 
	...
	var allPromise = this.q.all([ 
		...
		this.googleImageSearch(needle) 
			...
			function(result){
				...
				imagesList = result[1];
				// a new needle has been analyzed, search result are returned
				deferred.resolve({'web':webpagesList, 'img':imagesList});
			},
			function(error){
				deferred.reject("Error during search: "+error);
			}
		).done();
